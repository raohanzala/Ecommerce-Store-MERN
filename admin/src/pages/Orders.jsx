...
const Orders = ({ token }) => {
  ...
  const { isLoading, setIsLoading, currentPage, totalPages, setPageTitle, timestampToShortDate, orders, fetchAllOrders } = useContext(ShopContext)
            ...
            {orders?.length > 0 ? (
              orders.map((order, index) => (
                ...
                >
                  ...
                  <td
                    className={`py-3 border px-4 text-sm font-semibold ${order.status === 'Pending'
                      ? 'text-yellow-500'
